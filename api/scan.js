export default async function handler(req, res) {
  ...
  const SECTORS = {
    sp500: ["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","JPM","V","UNH","XOM","LLY","JNJ","WMT","MA","PG","HD","MRK","AVGO","CVX","PEP","COST","ABBV","KO","ADBE","CRM","BAC","MCD","CSCO","PFE","ABT","NKE","ORCL","TXN","NEE","HON","IBM","AMGN","GE","QCOM","LOW","INTU","CAT","GS","MS","BKNG","AXP","BLK","ISRG","SYK","TJX","ADP","VRTX","ADI","REGN","ZTS","CI","SO","MO","EOG","SLB","NOC","ITW","CME","CL","USB","PNC","TGT","EMR","MCO","F","GM","CTAS","WM","APD","NSC","HCA","ECL","DUK","WFC","C","DVN","MPC","VLO","OXY","NEM","SHW","PPG","NUE","BA","CMG","COP","DE","DIS","EW","HAL","HIG","HLT","ICE","LIN","LMT","MAR","MET","NTRS","PAYX","PRU","RMD","ROK","ROST","SBUX","SPG","STZ","SWK","SYF","T","TDG","TFC","TMUS","TROW","TRV","TSCO","UPS","VZ","WAT","WEC","WELL","XEL","YUM","ZTS"],
    nasdaq: ["NFLX","PYPL","INTC","AMD","AMAT","KLAC","LRCX","MRVL","PANW","SNPS","CDNS","FTNT","TEAM","ZS","DDOG","CRWD","OKTA","MDB","NET","SNOW","ABNB","DASH","COIN","RBLX","SOFI","AFRM","UPST","MELI","UBER","LYFT","EXPE","DKNG","CMCSA","SNAP","PINS","SPOT","BIDU","JD","PDD","NIO","XPEV","LI","RIVN","LCID","IONQ","RKLB","ACHR","JOBY","BILI","WDAY","VEEV","HUBS","DXCM","ALGN","LULU","NXPI","SMCI","HOOD","PLTR","TTD","TWLO","BILL","MNDY","TOST"],
    semis: ["NVDA","AMD","INTC","QCOM","AVGO","TXN","AMAT","KLAC","LRCX","MRVL","ON","SWKS","QRVO","MPWR","WOLF","ACLS","COHU","FORM","ICHR","MKSI","AMBA","SLAB","DIOD","SITM","MTSI","AEHR","POWI","LSCC","RMBS","ONTO","FSLR","ENPH","SEDG","SPWR","CSIQ","BRKS","CAMT","ENTG","IPGP","KLIC"],
    pharma: ["PFE","MRK","ABBV","LLY","BMY","AMGN","GILD","BIIB","REGN","VRTX","MRNA","BNTX","INCY","BMRN","ALNY","IONS","SRPT","ARWR","NTLA","BEAM","EDIT","CRSP","SGEN","EXEL","JAZZ","ITCI","ACAD","HALO","RARE","FOLD","ARCT","APLS","TGTX","AGIO","KYMR","IMVT","NKTR","RCKT","FATE","BLUE","VCEL","NBIX","INSM","SAGE","RXRX"],
  ...
  async function fetchTicker(ticker) {
    try {
      ...
      const e10 = calcEMA(closes,10); const e20 = calcEMA(closes,20);
      ...
      return {
        ticker, name: result.meta?.shortName||ticker, sector: label,
        price: Math.round(cur*100)/100, ema10: Math.round(e10*100)/100,
        ema20: Math.round(e20*100)/100, dist10:d10, dist20:d20, rsi, rel_vol:rv,
        change1d:c1, change5d:c5, score,
  ...
  for (let i = 0; i < tickers.length; i += batchSize) {
    ...
    const batchResults = await Promise.all(batch.map(t => fetchTicker(t)));
    batchResults.forEach(r => { if(r && Math.abs(r.dist10)>=minDist) results.push(r); });
  }

  results.sort((a,b) => b.score-a.score);
  res.json({
    ok:true, sector:label, total_scanned:tickers.length,
    found:results.length, timestamp:new Date().toISOString(),
